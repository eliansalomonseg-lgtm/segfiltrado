...
class EscuelaController
{
    ...
    private function prepararTablaEscuelas(PDO $conexion): void
    {
        ...
        foreach ($consulta->fetchAll() as $fila) {
            $existentes[strtoupper((string) $fila['Field'])] = strtolower((string) $fila['Type']);
        }
        foreach ($columnas as $columna => $definicion) {
            ...
            } elseif (str_starts_with($existentes[$columna], 'varchar') && str_contains($definicion, 'TEXT')) {
                $conexion->exec('ALTER TABLE escuelas MODIFY COLUMN `' . $columna . '` ' . $definicion);
            }
        }
    }
