...
function tarjetaEscuelaSugerida(escuela, rpu, atributo) {
    ...
    const tipoCandidato = escuela.grupo || (escuela.administrativa ? 'INMUEBLE SEG' : 'ESCUELA');
...
function renderRpuMatch(data) {
    ...
    const title = vinculados.length ? 'Agregar otro vínculo al RPU' : 'Opciones del padrón';
...
function renderSugerencias(data) {
    ...
    suggestionList.innerHTML = coincidencias.length ? coincidencias.map((item) => {
        ...
        return `<article class="suggested-rpu-card"><div class="suggested-rpu-head"><strong>${matchEscape(item.rpu)}</strong><span class="status-pill">${matchEscape(cfe.tarifa || 'N/D')}</span></div><div class="suggested-cfe-data"><strong>${matchEscape(cfe.nombre || 'Sin nombre CFE')}</strong><small>${matchEscape(cfe.direccion || 'Sin dirección')} · ${matchEscape(cfe.poblacion || 'Sin población')}</small><small>${matchEscape(cfe.division || 'Sin división')} · ${matchEscape(cfe.periodo || 'Sin periodo')}</small></div><div class="match-suggestions"><div class="match-title"><strong>Opciones del padrón</strong><span>${opciones.length} sugerencias</span></div>${opciones.length ? opciones.map((escuela) => cardEscuelaSugerida(escuela, item.rpu)).join('') : `<button class="manual-suggest-search" type="button" data-open-manual-rpu="${matchEscape(item.rpu)}"><i class="bi bi-search me-1"></i>Buscar este RPU manualmente</button>`}${controlCctManual(item.rpu)}</div></article>`;
